...
export const ProductUploadModal: React.FC<ProductUploadModalProps> = ({
  ...
  const modalContent = (
    ...
    >
      <div
        className="glass-secondary text-black border border-white/20 rounded-lg sm:rounded-xl lg:rounded-2xl p-4 sm:p-5 md:p-6 w-full max-w-xs sm:max-w-2xl lg:max-w-4xl max-h-[calc(100vh-120px)] overflow-y-auto hide-scrollbar overscroll-contain touch-pan-y"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-center justify-between mb-4 sm:mb-6 gap-4">
          <div className="min-w-0 flex-1">
            <h2 className="text-lg sm:text-xl md:text-2xl font-semibold" id="product-upload-title">Product Upload</h2>
            <p className="text-xs sm:text-sm text-black/60 mt-1">{mode === 'edit' ? 'Update your uploaded product.' : 'Add new products tied to destinations.'}</p>
          </div>
          <button
            type="button"
        ...
        <form className="flex flex-col gap-4 sm:gap-5 md:gap-6" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between gap-2">
              <label className="text-xs sm:text-sm text-black/60">Product name</label>
            ...
            <input
              ...
              className="rounded-lg bg-white/10 border border-[#1A1A1A]/15 px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm text-black placeholder:text-black/70 focus:outline-none focus:ring-2 focus:ring-white/30"
            />
          </div>
          <div className="flex flex-col gap-2">
            {isMunicipalityUser && (
              <div className="flex flex-col gap-2 rounded-lg bg-white/5 border border-white/10 p-3 sm:p-4">
                ...
                <div className="rounded-lg bg-white/10 border border-[#1A1A1A]/15 px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm text-black">
                  {profile?.municipality_name || 'N/A'}
                </div>
                <p className="text-xs text-black/50">Your municipality is pre-selected and cannot be changed.</p>
              </div>
            )}
            <label className="text-xs sm:text-sm text-black/60 mt-1">Location</label>
            <LocationPickerMap 
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="product-upload-images" className="text-xs sm:text-sm text-black/60">Image upload</label>
            <input
              ...
              className="rounded-lg bg-black/5 border border-[#1A1A1A]/15 px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm text-black file:mr-2 sm:file:mr-3 file:rounded-full file:border-0 file:bg-black/10 file:px-2 sm:file:px-3 file:py-1 file:text-xs file:text-black"
            />
